...
function filterContent(_ident, _filter){
    ...
    switch(_ident){
        ...
        case 'friends':
            ...
            break;
        case 'props':
            var list = filterList(propList, _filter);
...
function displaySearch(_results){
    ...
    for(var i=0; searchProps[i]; i++){
        ...
        propHtml += '<div class="content-cell prop"><div class="content-cell-formatter"></div>'+
            ...
            '<div class="content-btn button first" onclick=\'SelectProp(\"'+searchProps[i].ResultId+'\", \"'+GetCachedImage(searchProps[i].ResultImageCoui, searchProps[i].ResultImageUrl)+'\", \"'+searchProps[i].ResultName.replace(/"/g, '-').makeSafe()+'\");\'>Select Prop</div>'+
...
function AddProp(_prop){
    ...
    var html = '<div id="prp_'+_prop.SpawnableId+'" class="content-cell prop"><div class="content-cell-formatter"></div>'+
        ...
        _prop.SpawnableName.makeSafe()+'</div><div class="content-btn button first zero" onclick="getPropDetails(\''+_prop.SpawnableId+'\');">Details</div>'+
        '<div class="content-btn button first" onclick=\'SelectProp(\"'+_prop.SpawnableId+'\", \"'+spawnableImageUrl+'\", \"'+_prop.SpawnableName.replace(/"/g, '-').makeSafe()+'\");\'>Select Prop</div>'+
...
function inp_slider(_obj){
    ...
    this.valueLabelForeground.setAttribute('style', 'width: '+(this.value == 0 ? 0 : (1.0 / ((this.value - this.minValue) / (this.maxValue - this.minValue)) * 100))+'%;');
...
function LazyLoadImages() {
    ...
    if (images.length > 0 && images[0].getAttribute('data-loading') != 'loading'){
        ...
        // Iterate images until we reach one that requires lazy loading
